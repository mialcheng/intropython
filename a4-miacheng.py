...
def add_text_to_frequency_dict(freq_dict, text):
	...
	* if it isn't, set the value to 1
	...
	token = tokenize(text)
	for each in token:
		if freq_dict.get(each) != None:
			freq_dict[each] = freq_dict.get(each) + 1
		else:
			freq_dict[each] = 1
	return None
...
def get_total_words(freq_dict):
	...
	return sum(list(freq_dict.values()))
...
class BayesClassifier:
	...
	def train(self, directory_name): 
		...
		for file_name in file_names:
			if file_name.endswith(".txt"):
				...
				with open(directory_name + "/" + file_name, encoding='utf8') as f:
					review_string = f.read()
				if file_name.startswith(self.pos_file_prefix):
					add_text_to_frequency_dict(self.pos_freqs,review_string)
				elif file_name.startswith(self.neg_file_prefix):
					add_text_to_frequency_dict(self.neg_freqs, review_string)
	...
	def get_neg_frequency(self, word):
		...
		if self.neg_freqs.get(word) != None:
			return self.neg_freqs.get(word)
		else:
			return 0
	...
	def get_pos_frequency(self, word):
		...
		if self.pos_freqs.get(word) != None:
			return self.pos_freqs.get(word)
		else:
			return 0
	...
	def save(self, filename): 
		...
		with open(filename, 'w', encoding='utf8') as f:
			json.dump({"pos":self.pos_freqs, "neg": self.neg_freqs}, f, indent=4, sort_keys=True) 
		return None
	...
	def load(self, filename): 
		...
		with open(filename, encoding='utf8') as f:
			dict = json.load(f)
			self.neg_freqs = dict.get("neg")
			self.pos_freqs = dict.get("pos")
		return None
	...
	def get_likelihood_of_word(self, word, is_positive):
		...
		if is_positive:
			return float(self.get_pos_frequency(word) + 1)  /  get_total_words(self.pos_freqs)
		else:
			return float(self.get_neg_frequency(word) + 1)  /  get_total_words(self.neg_freqs)
	...
	def get_log_likelihood_of_text(self, text, is_positive):
		...
		tok = tokenize(text) #tokenize
		probs = [math.log(self.get_likelihood_of_word(each,is_positive)) for each in tok]
		return sum(probs)
		#changed tol to e-03
	...
	def get_prediction(self, text):
		...
		pos = self.get_log_likelihood_of_text(text, True)
		neg = self.get_log_likelihood_of_text(text, False)
		if  neg <= pos:
			return "positive"
		else:
			return "negative"
...
if __name__ == "__main__":
	...
	assert math.isclose(bad_review_likelihood_pos, -18.2799297, rel_tol=1e-3, abs_tol=0.0)
	assert math.isclose(bad_review_likelihood_neg, -14.8187559, rel_tol=1e-3, abs_tol=0.0)
