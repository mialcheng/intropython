...
def get_breed_data_by_name(breeds, query_name):
	...
	for breed in breeds:
		if is_breed_match(breed[0], query_name):
			return breed
	return None
...
def get_dogs_by_breed(dogs, breed_name):
	...
	ls = []
	for dog in dogs:
		if is_breed_match(breed_name, dog[INDEX_DOG_BREED]):
			ls.append(dog)
	return ls
...
def get_names_by_breed(dogs, breed_name):
	...
	ls = []
	for dog in get_dogs_by_breed(dogs, breed_name):
		ls.append(dog[INDEX_DOG_NAME])
	return sorted(ls)
...
def has_temperament(breed_data, adjective):
	...
	if breed_data == None or breed_data == []:
		return False
...
def list_all_zip_codes(dogs):
	...
	ziplist = []
	for dog in dogs:
		if dog[INDEX_DOG_ZIPCODE] not in ziplist:
			ziplist.append(dog[INDEX_DOG_ZIPCODE])
	return sorted(ziplist)
...
def list_breeds(dogs):
	...
	breedlist = []
	for dog in dogs:
		if dog[INDEX_DOG_BREED] not in breedlist:
			breedlist.append(dog[INDEX_DOG_BREED])
	return sorted(breedlist)
...
def search_dogs(dogs, breeds, query_zip_code=None,query_positive_traits=None,query_negative_traits=None,query_name=None,query_breed=None):
	...
	def dog_is_match(dog):
		...
		if query_zip_code != None:
			match = match and query_zip_code in dog[INDEX_DOG_ZIPCODE]
		if query_name != None:
			match = match and query_name.upper() == dog[INDEX_DOG_NAME]
		if query_breed != None:
			match = match and is_breed_match(dog[INDEX_DOG_BREED], query_breed)
		if query_positive_traits != None:
			match = match and all([has_temperament(get_breed_data_for_dog(breeds, dog), trait) for trait in query_positive_traits])
		if query_negative_traits != None:
			match = match and not any([has_temperament(get_breed_data_for_dog(breeds, dog), trait) for trait in query_negative_traits])
