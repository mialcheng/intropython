...
def load_grammar(path_to_grammar):
	...
	with open(path_to_grammar) as file:
...
def get_key_count(grammar):
	...
	return sum([1 for key in grammar.keys()])
...
def apply_modifiers(modifier_names, text):
	...
	for each in modifier_names:
		text = modifier_dictionary[each](text)
	return text
...
def expand_rule_section(grammar, section_text, index):
	...
	*** A helper function for expand_text *** 
	...
	if index%2 == 0: 	# For even sections (index%2==0), return the original section text (we don't need to do anything further)
		return section_text	
	else: 	# For odd sections, this is a *socket* (ie, "animal.a.capitalize", and we need to figure out what to fill it with
		split = section_text.split(".") #  * split the socket text with "." to get a list (https://www.programiz.com/python-programming/methods/string/split)
		getrule = get_rule(grammar, split[0])  #the "key" is the *first* element in the list (ie: "animal")
		left = split[1:] # the elements left are a list of modifier 
		
		if "".join(getrule.split("#")).strip() == getrule:
			return apply_modifiers(left, getrule)
		else:
			return apply_modifiers(left, expand_rule(grammar, getrule))
...
def expand_rule(grammar, rule):
	...
	* Use "".join(some_list) to merge the list back into a single string
	...
	split = rule.split("#")
	newlist = "".join([expand_rule_section(grammar, item, split.index(item)) for item in split])
	return newlist
...
def respond_to_input(grammar, user_input):
	...
	if "unknown" in grammar.keys():
		return expand_rule(grammar, get_rule(grammar, "unknown")) 
	else:
...
my_grammar = {
	"person": ["ella", "jenny", "danny", "cheryl", "david"],
	"food": ["pizza", "smoothies", "sushi", "ramen"],
	"tv_show" : ["criminal minds", "sherlock"],
	"topic": ["candle", "book", "#food# recipe", "#tv_show# episode summary"],
	"idea": "#topic.a#",
	"origin": "one day, #person# and #person# were eating #food# and they decided to find #idea#"
}
#testing for requirements 
assert get_key_count(my_grammar) >= 5
assert get_rule_count(my_grammar) >= 15
response0 = respond_to_input(my_grammar, "Tell me about my origin story") 
print(response0)
