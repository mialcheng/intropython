...
def get_title(soup):
	...
	return soup.title.text[0:soup.title.text.find(" - Wikipedia")]
...
def get_last_edit(soup):
	...
	* Finally, we don't want the whole text, 
	...
	date = soup.find(id='footer-info-lastmod')
	s_in = date.text.find(start_text)
	e_in = date.text.find(end_text)
	return date.text[s_in+4:e_in]
...
def get_wordcount(soup):
	...
	**To get just the text of a whole page, use "soup.text"**
	...
	num = sum([1 for word in soup.text.split()])
	return num
...
def get_all_links(soup):
	...
	* and return a list of all the "href" attributes (the link urls)
	...
	url = []
	for u in a:
		if "href" in u.attrs and u.attrs["href"] != "":
			url.append(u.attrs["href"])
	return url
...
def get_all_wiki_link_ids(soup):
	...
	* don't have ":" or "#"
	...
	return [link[link.find('/wiki/')+6:] for link in get_all_links(soup) if "#" not in link and ':' not in link]
...
class WikiPage:
	def __init__(self, page_directory, page_id):
		...
		load_url = load_url_politely(self.url)
		self.soup = BeautifulSoup(load_url, 'html.parser')
	...
	def load_links(self, recursion_count=0, links_per_article=5, randomize_links=False):
		...
		intlinks = self.get_interesting_links() 
		if randomize_links == True:
			random.shuffle(intlinks)

		# for i in intlinks:
		# 	if i.count('dex.php?title=Cat&action=info') != 0:
		# 		intlinks.remove(i)
		# print("links per artcle")
		# print(links_per_article)
		# print(randomize_links)
		# print('this is int links')
		# print(intlinks)
		short = intlinks[0:links_per_article]
		# print("THIS IS SHORT")
		# print(short)\
		recursion_count_for_next_level = recursion_count - 1

		for wiki_id in short:
			if self.page_directory.get(wiki_id) == None:
				page = WikiPage(self.page_directory,wiki_id)
				if recursion_count != 0:
					return page.load_links(recursion_count_for_next_level,links_per_article)
			# else:
...
if __name__ == "__main__":
	...
	assert "Istanbul" in page_directory, "If it loaded the links 3 links deep from Cat, 'Istanbul' should be in the directory"
