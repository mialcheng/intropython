...
class Student: 
	...
	** Review https://www.programiz.com/python-programming/class
	...
	def propose_to_top_choice(self):
		...
		topname = self.to_propose.pop()
		top = self.all_students.get_student_by_name(topname)

		if not top.has_partner():
			self.make_partnership(top)
			self.to_propose.append(top.name)
		else:
			if top.get_rating_of_current_partner() < top.get_rating_of_name(self.name):
				self.make_partnership(top)
				self.to_propose.append(top.name)
...
class Group: 
	...
	def make_naive_partnerships(self):
		...
		for ii, a  in enumerate(self.students_a, 0):
			a.make_partnership(self.students_b[ii])
		return None
	...
	def make_gale_shapely_partnerships(self):
		...
		#break all existing partnerships 
		self.break_all_partnerships()
		#add to propose list to each student in group a 
		# to_propose = self.partner_ratings[:]
		for proposer in self.students_a:
			proposer.to_propose = proposer.partner_ratings[:]
		while not all([proposer.has_partner() for proposer in self.students_a]):
			[proposer.propose_to_top_choice() for proposer in self.students_a if not proposer.has_partner()]
			
		
		# while not all([proposer.has_partner() for proposer in self.students_a]):
		# 	for proposer in self.students_a:
		# 		proposer.to_propose = proposer.partner_ratings[:] 
		# 		if not proposer.has_partner():
		# 			proposer.propose_to_top_choice()
		...
		# while not all([proposer.has_partner() for proposer in self.students_a]):
		# 	#print([proposer.has_partner() for proposer in self.students_a])
		# 	for proposer in self.students_a:
		# 		proposer.to_propose = proposer.partner_ratings[:]
		# 		if not proposer.has_partner():
		# 			proposer.propose_to_top_choice()

			#haspart = [proposer.has_partner() for proposer in self.students_a]
			##print(all(haspart))
...
def run_experiment(student_count=10, run_count=10, matchmaking_fxn="make_gale_shapely_partnerships"):
	...
	for i in range(0,run_count):
		g.randomize_ratings()
		if matchmaking_fxn == 'make_gale_shapely_partnerships':
			g.make_gale_shapely_partnerships()
		elif matchmaking_fxn == 'make_naive_partnerships':
			g.make_naive_partnerships()
		#getattr(g, matchmaking_fxn) #call match metthod for the group
		total_happiness_a = total_happiness_a + calculate_average_happiness([g.get_student_by_name(name) for name in names_a])
		#print(total_happiness_a)
		total_happiness_b = total_happiness_b + calculate_average_happiness([g.get_student_by_name(name) for name in names_b])
	total_happiness += (total_happiness_a + total_happiness_b)/2
	...
	total_time = (end-start)/run_count
...
if __name__ == '__main__':
	...
	#student w no partner already
	mine = Student(None, "Riley", [])
	mine.break_partnership()
	assert not mine.has_partner()
