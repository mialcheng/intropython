...
def load_color_dataset(path):
	...
	colors = json.load(open("xkcd-colors.json", encoding="utf-8"))['colors'] #list of dicts 
...
def get_colors_by_word(all_colors, word):
	...
	# print(word)
	ls = [color for color in all_colors if word in color.name]
	if ls == []:
		return None 
	# print([color.name for color in ls])
	# print('sorted')
	ls.sort(key=lambda color: len(color.name))
	return ls
...
def draw_scatterplot(axes, colors, marker="o"):
	...
	for color in colors:
		...
		b = color[2]
		b1 = b/255
		axes.scatter3D(r,g,b,color=[r1,g1,b1], marker=marker)
...
def draw_colors(image, rect_size=(100, 200), rgb_colors=[], labels=None, font=None):
	...
	for (i,color) in enumerate(rgb_colors):
		...
		if labels != None:
			color_name = labels[i]
			im_draw.text(xy, color_name, fill=color, font=font)
		else:
			im_draw.rectangle(xy, fill=color)
...
def sort_cats(all_cats):
	...
	def get_num_cats_detected(catpic):
		if not hasattr(catpic, "detected_cats"):
			return 0
		else:
			return len(catpic.detected_cats)

	all_cats.sort(key=get_num_cats_detected, reverse=True)
	return all_cats[:]
...
class Color: 
	...
	def get_distance_to(self, rgb):
		...
		d_red = self.rgb[0] - rgb[0]
		d_green = self.rgb[1] - rgb[1]
		d_blue = self.rgb[2] - rgb[2]
		sum_colors = d_red**2 + d_green**2 + d_blue**2
		return math.sqrt(sum_colors)
...
class Cat_Picture:
	...
	def create_thumbnail(self, thumbnail_base=200):
		...
		sq = math.sqrt(self.aspect_ratio)
		w = round(thumbnail_base*sq)
		h = round(thumbnail_base/sq)
		img.thumbnail([w,h], resample=resampling_technique, reducing_gap=2.0)
		# -- Your code here --
		return img
	...
	def set_palette(self, named_colors,cluster_count=7):
		...
		# -- Your code here --
		calc_clust = calculate_clusters(pixels, cluster_count)
		self.palette = [get_closest_named_colors(named_colors, color, 1)[0] for color in calc_clust]
		return None
	...
	def draw_detected_cats(self):
		...
		bool_hasattr = hasattr(self, "detected_cats")
		if bool_hasattr == False:
			self.detect_cats()
		...
		# # load the cat detector Haar cascade, then detect cat face # in the input image
		# rects = cv2.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=10, minSize=(75, 75))

		
		copy = self.full_image.copy()
		draw = ImageDraw.Draw(copy)

		for box in self.detected_cats:
			draw.rectangle((box[0],box[1],box[0]+box[2],box[1]+box[3]), outline=(255,255,255), width=10)
		# -- Your code here --
...
if __name__ == "__main__":
	...
	# print('this is first colors')
	# print(first_colors[0])
